...
describe("favoriteService", () => {
  ...
  describe("add", () => {
    ...
    it("planId 为空时抛 BAD_REQUEST", async () => {
      await expect(favoriteService.add(mockPrisma, "user-1", "")).rejects.toThrow(
        new TRPCError({ code: "BAD_REQUEST", message: "planId 不能为空" })
      );
    });
  });
  ...
  describe("remove", () => {
    ...
    it("收藏不存在时抛 NOT_FOUND", async () => {
      ...
      await expect(favoriteService.remove(mockPrisma, "user-1", "plan-1")).rejects.toThrow(
        new TRPCError({ code: "NOT_FOUND", message: "收藏不存在" })
      );
    });

    it("planId 为空时抛 BAD_REQUEST", async () => {
      await expect(favoriteService.remove(mockPrisma, "user-1", "")).rejects.toThrow(
        new TRPCError({ code: "BAD_REQUEST", message: "planId 不能为空" })
      );
    });
  });
  ...
  describe("sync", () => {
    it("合并本地和服务器收藏", async () => {
      ...
      mockPrisma.favorite.findMany
        .mockResolvedValueOnce([{ planId: "plan-1", imageUrl: null }])
        // 合并后返回完整列表
