...
describe('PlanCard', () => {
  ...
  describe('商家信息', () => {
    it('showMerchant=true 时显示商家名称', () => {
      render(
        <PlanCard
          plan={{ ...basePlan, merchantName: '京都本店' }}
          showMerchant={true}
        />
      );
  ...
  describe('价格展示', () => {
    it('有折扣时显示原价和省钱标签', () => {
      render(
        <PlanCard plan={{ ...basePlan, originalPrice: 1200000 }} />
      );
      // 原价 ¥12,000
    ...
    it('originalPrice <= price 时不显示折扣', () => {
      render(
        <PlanCard plan={{ ...basePlan, originalPrice: 980000 }} />
      );
      expect(screen.queryByText(/省¥/)).not.toBeInTheDocument();
    });
  });
  ...
  describe('包含项', () => {
    it('显示前两项包含物', () => {
      render(
        <PlanCard plan={{ ...basePlan, includes: ['和服', '腰带', '发型'] }} />
      );
