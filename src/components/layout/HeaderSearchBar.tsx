...
export default function HeaderSearchBar() {
  ...
  const mobileSearchButton = (
    <button
      ...
      className="md:hidden flex items-center gap-2 bg-white border border-gray-300 rounded-full px-4 py-2 shadow-sm active:scale-95 transition-all"
    >
      <Search className="w-4 h-4 text-gray-600" />
      <span className="text-sm text-gray-600 font-medium">搜索和服体验</span>
    </button>
  );

  // 移动端全屏搜索模态框
  const mobileSearchModal = isMobileModalOpen && (
