...
export function HeroSearchBar() {
  ...
  const { location, setLocation, date, setDate, theme: themeSlug, setTheme: setThemeSlug } = useSearchState();
  const [isPending, startTransition] = useTransition();

  const [mobileExpanded, setMobileExpanded] = useState(false);

  // Theme 对象状态（从 slug 解析）
  const [themes, setThemes] = useState<Theme[]>(cachedThemes || []);
  ...
  // 获取 themes 并同步 selectedTheme
  useEffect(() => {
    if (cachedThemes) {
      setThemes(cachedThemes);
      if (themeSlug) {
        const found = cachedThemes.find((t) => t.slug === themeSlug);
    ...
    fetch("/api/themes")
      ...
      .then((data) => {
        ...
        if (themeSlug) {
          const found = fetchedThemes.find((t: Theme) => t.slug === themeSlug);
          setSelectedTheme(found || null);
        }
      })
      .catch((error) => {
  ...
  const handleThemeChange = async (theme: Theme | null) => {
    ...
    await setThemeSlug(theme?.slug || null);
  };

  const handleSearch = () => {
    // URL 已经由 nuqs 管理，直接导航到 /plans
  ...
  return (
    <>
      <div className="w-full max-w-4xl mx-auto">
        {/* 桌面端：横向展开搜索框 */}
        <div
          className="hidden md:flex rounded-full p-2 gap-2 items-center bg-white border border-gray-200
        ...
        >
              ...
              >
                <div className="py-2">
                  {filteredLocations.map((loc, index) => (
                    ...
                    >
                      <div
                        className="flex-shrink-0 w-10 h-10 rounded-xl bg-gray-50 flex items-center justify-center
          ...
          {/* 主题 */}
          <div className="flex-1 px-6 py-3 rounded-full hover:bg-gray-100/50 transition-all duration-200 group relative cursor-pointer">
