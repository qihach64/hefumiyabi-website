...
interface ThemeImageSelectorProps {
  ...
  isPending?: boolean;
  pendingThemeSlug?: string | null;
}

export function ThemeImageSelector({
  themes,
  selectedTheme,
  onSelect,
  isPending = false,
  ...
  // 判断是否有正在进行的切换
  const hasPendingSwitch = isPending && pendingThemeSlug !== undefined;
  ...
  return (
    <div className="relative">
      ...
      >
        {/* 全部选项 */}
        {(() => {
          const isLoadingThis = hasPendingSwitch && pendingThemeSlug === null;
        ...
        {/* 主题选项 */}
        {themes.map((theme) => {
          const isLoadingThis = hasPendingSwitch && pendingThemeSlug === theme.slug;
          const isSelected = hasPendingSwitch
            ? pendingThemeSlug === theme.slug
